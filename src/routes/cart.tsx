  ...
  if (items.length === 0) {
    return (
      <Container className="py-20 text-center">
        ...
        <Link
          to="/catalog"
          className="mt-6 inline-flex items-center rounded-full bg-foreground px-6 py-3 text-sm font-medium text-background"
        >
          В каталог
        </Link>
      </Container>
      ...
      <div className="mt-8 grid gap-10 lg:grid-cols-[1fr_360px]">
        ...
        <aside className="space-y-4 lg:sticky lg:top-24 lg:self-start">
          ...
          <div className="rounded-2xl border border-border/60 bg-card p-5">
            ...
            <Link
              to="/checkout"
              className="mt-5 inline-flex w-full items-center justify-center rounded-full bg-foreground px-6 py-3 text-sm font-medium text-background hover:bg-foreground/90"
            >
              Оформить заказ
            </Link>
            <Link
